"""
---connect_realsense_v1.2---
xArmに取り付けられたrealsenseからの映像をフィードバックしつつ台車およびアームの操作が可能
...
def xs_control(task):
    ...
    rbt_s.fk(jnt_values=rbt_x.arm_get_jnt_values())
    onscreen_rbt.detach()
    onscreen_rbt = rbt_s.gen_meshmodel()
    onscreen_rbt.attach_to(base)
...
if __name__ == '__main__':
    ...
    taskMgr.doMethodLater(update_frequency, get_vision, "get_vision",
                          extraArgs=None,
                          appendTask=True)
    taskMgr.doMethodLater(update_frequency, xs_control, "xs_control",
                          extraArgs=None,
                          appendTask=True)
    base.run()
