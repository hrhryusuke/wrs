...
20221204
"""
# TODO: 両方のマニピュレータを同時操作するための機能の実装
...
rbt_s1 = xsm.XArmShuidi(enable_cc=True)
rbt_s2 = xsm.XArmShuidi(enable_cc=True)
homeconf_jnt_values = rbt_s1.get_jnt_values()
...
def independent_agv_move(task):
    ...
    if mode_independent is True or mode_manipulation is True:
        ...
        if pressed_keys1["i"] and pressed_keys1["j"]:
            ...
            if camera_robot == 1:
                rel_rot = rm.rotmat_from_euler(0, arm_angular_speed * .5, 0)
            else:
    ...
    if mode_manipulation is True:
        if count_prepare == 0:
            if camera_robot == 1:
                ...
                rbt_s2.fk(jnt_values=init_jnt_values_manipulation)
            else:
                rbt_x1.arm_move_jspace_path(path=[rbt_x1.arm_get_jnt_values(), init_jnt_values_manipulation])
                rbt_s1.fk(jnt_values=init_jnt_values_manipulation)
...
def arm_move(rbt_s1, rbt_s2, onscreen1, onscreen2, task):
    ...
    agv_pos_and_angle = rbt_s2.get_jnt_values("agv")
    ...
    if is_ready_manipulation == True:
        ...
        model_remove(onscreen1_tcpframe)
        ...
        if operation_count == 0:
            tcp_pos, tcp_rot = rbt_s2.get_gl_tcp()
            ...
            # マニピュレータの手先の座標系表示
            onscreen1_tcpframe.append(gm.gen_frame(pos=tcp_pos, rotmat=tcp_rot, length=0.3))
            onscreen1_tcpframe[-1].attach_to(base)
            ...
            onscreen1.append(rbt_s.gen_meshmodel())
            onscreen1[-1].attach_to(base)
