"""
---Operaing_xArmShuidi_sim_v1概要説明---
...
threshold_jnt_values_displacement_norm = 0.05
...
def arm_move(rbt_s, onscreen, task):
    ...
    if operation_count == 0:
        ...
        pre_sensor_qua = data_array[(rgt_hand_num + 6):(rgt_hand_num + 10)]
        if np.linalg.norm(rel_world_euler, ord=2) > 0:
        ...
        if (current_jnt_values is not None) and (pre_jnt_values is not None) and (collided_result == False):
            if abs(np.linalg.norm(current_jnt_values-pre_jnt_values, ord=2)) <= threshold_jnt_values_displacement_norm and np.all(abs(current_jnt_values-pre_jnt_values) <= 0.1):
