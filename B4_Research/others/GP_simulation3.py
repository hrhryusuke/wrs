...
WASDキーで台車部分の操作が可能
tcpの軌跡と操作者の右手の軌跡をプロットする（赤：tcpの軌跡，緑：操作者の右手の軌跡）
...
base = wd.World(cam_pos=[0, 0, 4], lookat_pos=[0, 0, 1]) # モデリング空間を生成
...
onscreen_tcpframe = []
onscreen_trajectory_tcp = []
onscreen_trajectory_operator = []
...
def arm_move(rbt_s, onscreen, task):
    global data_array, current_jnt_values_rgt, pre_jnt_values_rgt, fail_IK_flag, operation_count, modify_pos, modify_rot, pre_tcp_pos, pre_sensor_pos_value, \
        current_tcp_rot_euler, pre_tcp_rot_euler, init_pos_error
    ...
    if operation_count == 0:
        ...
        init_pos_error = data_array[rgt_hand_num:(rgt_hand_num+3)] - np.dot(rm.rotmat_from_euler(ai=0, aj=0, ak=-90), tcp_pos)
        ...
        if operation_count <= 100:
            onscreen_trajectory_tcp.append(gm.gen_sphere(pos=tcp_pos))
            onscreen_trajectory_tcp[-1].attach_to(base)
        ...
        current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=pre_tcp_pos + np.dot(modify_rot, rel_tcp_pos) + agv_pos,
                                                                           ...
                                                                           tcp_jntid=7)
        if operation_count <= 100:
            onscreen_trajectory_operator.append(gm.gen_sphere(pos=data_array[rgt_hand_num:(rgt_hand_num+3)] - init_pos_error, rgba=[0, 1, 0, 1]))
            onscreen_trajectory_operator[-1].attach_to(base)
