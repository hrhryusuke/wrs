...
# シミュレータ描画用関数
def display(attached_list, attached_list_tcp, rbt_s, onscreen, pre_pos, task):
    global k, pre_manipulability, current_manipulability, manipulability_displacement, data_array, pre_rgt_hand_info, current_rgt_hand_info, \
        current_jnt_values_rgt, displacement, pre_jnt_displacement, fail_IK_flag, operation_count, init_error_rgt, pre_tcp_pos
    ...
    if operation_count == 0:
        ...
        pre_rgt_hand_info = np.hstack([data_array[rgt_hand_num:(rgt_hand_num+3)] - init_error, np.array(list(tr.euler_from_quaternion(data_array[(rgt_hand_num+6):(rgt_hand_num+10)])))])
        ...
        # 制御処理
        current_rgt_hand_info = np.hstack([data_array[rgt_hand_num:(rgt_hand_num+3)] - init_error, np.array(list(tr.euler_from_quaternion(data_array[(rgt_hand_num+6):(rgt_hand_num+10)])))])
...
if __name__ == '__main__':
    ...
    taskMgr.doMethodLater(update_frequency, display, "display",
                          extraArgs=[attached_list, attached_list_tcp, rbt_s, onscreen, pre_agv_pos_rgt],
                          appendTask=True)
