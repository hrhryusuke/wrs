"""
---operate_xarm_actual_v1.0概要---
Axis NEURONから送信されてきたPERCEPTION NEURONの右手の位置姿勢情報を元に実機のxarmに動きを同期させる(1台のみ)
WASDキーで台車部分の操作が可能
...
rbt_s = xarm_sim.XArmShuidi(enable_cc=True) # ロボットモデルの読込
...
# 台車移動用関数
def agv_move(task):
    global onscreen, pre_agv_pos, current_jnt_values
    agv_speed_weight = .01
    ...
    if pressed_keys["w"] and pressed_keys["a"]:
        current_jnt_values = np.array(pre_agv_pos + [agv_speed_weight * agv_direction[0], agv_speed_weight * agv_direction[1], math.radians(agv_rotation_weight)])
        rbt_s.fk(component_name='agv', jnt_values=current_jnt_values)
        pre_agv_pos = current_jnt_values
        ...
        current_jnt_values = np.array(pre_agv_pos + [agv_speed_weight * agv_direction[0], agv_speed_weight * agv_direction[1], math.radians(0)])
        rbt_s.fk(component_name='agv', jnt_values=current_jnt_values)
        pre_agv_pos = current_jnt_values
        ...
        current_jnt_values = np.array(pre_agv_pos + [0, 0, math.radians(agv_rotation_weight)])
        rbt_s.fk(component_name='agv', jnt_values=current_jnt_values)
        pre_agv_pos = current_jnt_values
    ...
    if operation_count == 0:
        ...
        if abnormal_flag == False and current_jnt_values is not None:
            ...
            if collided_result == True:
                ...
                rbt_x.arm_move_jspace_path([pre_jnt_values, current_jnt_values])
