...
def init_operation(init_pos, init_rot):
    num_phase = 100
    ...
    specified_init_rot = np.array([[0, -1, 0],
                                   [-1, 0, 0],
                                   [0, 0, -1]])
    init_rot_displacement = specified_init_rot - init_rot
    phase_displacement = init_rot_displacement / num_phase
    for i in range(1, num_phase+1):
        IO_current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=init_pos,
                                                                              tgt_rot=init_rot + phase_displacement*i,
...
def display(rbt_s, onscreen, task):
    ...
    if operation_count == 0:
        ...
        init_operation(init_pos=tcp_pos, init_rot=tcp_rot)
    else:
        tcp_pos, tcp_rot = rbt_s.manipulator_dict['arm'].jlc.get_gl_tcp()
...
if __name__ == '__main__':
    ...
    # init_operation(init_pos=init_pos, init_rot=init_rot)
    threading.Thread(target=data_collect, args=(data_list,)).start()
    # threading.Thread(target=init_operation, args=(init_pos, init_rot,)).start()
