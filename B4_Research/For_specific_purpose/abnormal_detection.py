...
threshold_jnt_values_displacement_norm = 0.7
...
def arm_move(rbt_s, onscreen, task):
    global data_array, current_jnt_values_rgt, pre_jnt_values_rgt, fail_IK_flag, operation_count, modify_rot, modify_rot_for_pos, pre_tcp_pos, pre_sensor_pos_value, operator_coordinate, \
        init_error_rgt, pre_sensor_quarternion, pre_tcp_rot, init_tcp_rot, init_hand_rot, abnormal_flag, abnormal_count, stop_standard_pos_rgt, stop_standard_rot_rgt, cancel_pos_displacement_rgt, cancel_rot_displacement
    ...
    if operation_count == 0:
        ...
        gm.gen_frame(pos=np.zeros(3), rotmat=operator_coordinate, length=2, thickness=0.03).attach_to(base)
    # --------------------------------------------------------
    ...
    if operation_count == 0:
        ...
        init_hand_rot = rm.rotmat_from_quaternion(pre_sensor_qua)[:3, :3]
    else:
        tcp_pos, tcp_rot = rbt_s.manipulator_dict['arm'].jlc.get_gl_tcp()
        ...
        # -------------------------------------------------------
        rel_world_pos = data_array[rgt_hand_num:(rgt_hand_num + 3)] - pre_sensor_pos_value
        rel_world_euler = rm.quaternion_to_euler(data_array[(rgt_hand_num + 6):(rgt_hand_num + 10)]) - rm.quaternion_to_euler(pre_sensor_qua)
        pre_sensor_qua = data_array[(rgt_hand_num + 6):(rgt_hand_num + 10)]
        ...
        current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=np.dot(np.linalg.pinv(operator_coordinate), data_array[(rgt_hand_num):(rgt_hand_num + 3)]) - init_error + agv_pos - cancel_pos_displacement,
                                                                           tgt_rot=np.dot(cancel_rot_displacement, np.dot(np.linalg.pinv(operator_coordinate), rm.rotmat_from_quaternion(data_array[(rgt_hand_num+6):(rgt_hand_num+10)])[:3, :3])),
                                                                           seed_jnt_values=pre_jnt_values,
                                                                           tcp_jntid=7)
        # pre_jnt_values = current_jnt_values
        pre_tcp_rot = np.dot(np.linalg.pinv(operator_coordinate), pre_tcp_rot)
        # -------------------------------------------------------
        ...
        if abnormal_flag == 0:
            if (current_jnt_values is not None) and (pre_jnt_values is not None) and (collided_result == False):
                if abs(np.linalg.norm(current_jnt_values-pre_jnt_values, ord=2)) <= threshold_jnt_values_displacement_norm and np.all(abs(current_jnt_values-pre_jnt_values) <= 0.08):
            ...
            if abnormal_count == 1:
                standard_pos = np.dot(np.linalg.pinv(operator_coordinate), data_array[(rgt_hand_num):(rgt_hand_num + 3)]) - init_error + agv_pos - cancel_pos_displacement
                standard_rot = np.dot(np.linalg.pinv(operator_coordinate), rm.rotmat_from_quaternion(data_array[(rgt_hand_num+6):(rgt_hand_num+10)])[:3, :3])
                ...
                judge = abnormal_judgement(standard_pos=standard_pos, standard_rot=standard_rot,
                                            current_pos=np.dot(np.linalg.pinv(operator_coordinate), data_array[(rgt_hand_num):(rgt_hand_num + 3)]) - init_error + agv_pos - cancel_pos_displacement,
