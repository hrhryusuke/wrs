...
---modifying_coordinate概要説明---
Axis NEURONから送信されてきたPERCEPTION NEURONの右手の位置姿勢情報を元にシミュレータ上のxarmに動きを同期させる
WASDキーで台車部分の操作が可能
同次変換行列を用いた初期姿勢の補正機能を追加

BASE PROGRAM:robot_test_v9

---modifying_coordinate更新箇所---
...
def arm_move(rbt_s, onscreen, task):
    global data_array, current_jnt_values_rgt, pre_jnt_values_rgt, fail_IK_flag, operation_count, init_error_rgt, modify_homomat
    ...
    if operation_count == 0:
        ...
        # ☆新補正処理
        init_pos_error = data_array[rgt_hand_num:(rgt_hand_num+3)] - tcp_pos
        init_rot_error = np.dot(tcp_rot, np.linalg.pinv(rm.rotmat_from_quaternion(data_array[(rgt_hand_num+6):(rgt_hand_num+10)])[:3, :3]))
        modify_homomat = rm.homomat_from_posrot(pos=-init_pos_error, rot=init_rot_error)
        # print(modify_homomat)
        # -----------------------------
        ...
        # ☆新制御処理
        current_homomat = rm.homomat_from_posrot(pos=data_array[rgt_hand_num:(rgt_hand_num+3)], rot=rm.rotmat_from_quaternion(data_array[(rgt_hand_num+6):(rgt_hand_num+10)])[:3, :3])
        modified_homomat = np.dot(modify_homomat, current_homomat)
        # print(modified_homomat)
        print('tgt_pos')
        print(modified_homomat[:3, 3].T)
        print('tgt_rot')
        print(modified_homomat[:3, :3])
        print('\n')
        current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=data_array[rgt_hand_num:(rgt_hand_num+3)] - init_error,
                                                                           tgt_rot=modified_homomat[:3, :3],
                                                                           seed_jnt_values=pre_jnt_values,
                                                                           tcp_jntid=7)
        pre_jnt_values = current_jnt_values
        # -----------------------------
