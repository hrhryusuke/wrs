...
base = wd.World(cam_pos=[-3, 0, 1.5], lookat_pos=[0, 0, 1]) # モデリング空間を生成
...
def arm_move(rbt_s_rgt, rbt_s_lft, onscreen_rgt, onscreen_lft, task):
    ...
    if operation_count == 0:
        ...
        pre_sensor_pos_value_rgt = data_array[rgt_hand_num:(rgt_hand_num+3)]
        current_jnt_values_rgt = rbt_s_rgt.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=np.dot(agv_direction_rot_rgt, pre_tcp_pos_rgt + np.dot(modify_rot_rgt, rel_tcp_pos_rgt) - init_pos_rgt),
                                                                                   tgt_rot=np.dot(agv_direction_rot_rgt, np.dot(modify_rot_rgt, rm.rotmat_from_quaternion(data_array[(rgt_hand_num+6):(rgt_hand_num+10)])[:3, :3])),
                                                                                   seed_jnt_values=pre_jnt_values_rgt,
                                                                                   tcp_jntid=7)
        pre_jnt_values_rgt = current_jnt_values_rgt
        pre_tcp_pos_rgt = np.dot(agv_direction_rot_rgt, pre_tcp_pos_rgt + np.dot(modify_rot_rgt, rel_tcp_pos_rgt) - init_pos_rgt)
        print(pre_tcp_pos_rgt + np.dot(modify_rot_rgt, rel_tcp_pos_rgt) - init_pos_rgt)
        # left_arm
        tcp_pos_lft, tcp_rot_lft = rbt_s_lft.manipulator_dict['arm'].jlc.get_gl_tcp()
