...
def display(attached_list, rbt_s, onscreen, pre_pos, task):
    global k, pre_manipulability, current_manipulability, manipulability_displacement, data_array, pre_rgt_hand_info, current_rgt_hand_info, \
        current_jnt_values_rgt, displacement, pre_jnt_displacement, fail_IK_flag, operation_count
    ...
    if operation_count == 0:
        ...
        jnt_displacement = np.dot(test_arg2, displacement) + np.dot((np.eye(7)-np.dot(test_arg2, test_arg1)), k)
        ...
        if not (np.any(jnt_displacement > threshold_jnt_displacement)):
            rbt_s.fk("arm", current_jnt_values + jnt_displacement)
