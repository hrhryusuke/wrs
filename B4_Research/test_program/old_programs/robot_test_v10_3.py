...
def data_adjustment(data_list):
    ...
    for i in range(0, tmp_data_array.size):
        if (i % 16) == 2: # センサ座標調整
            ...
            tmp_euler = rm.rotmat_to_euler(np.dot(adjustment_rotmat3, rm.rotmat_from_euler(ai=gripper_euler[0], aj=gripper_euler[1], ak=gripper_euler[2])))
            gripper_euler[0] = -tmp_euler[2]
            gripper_euler[1] = -tmp_euler[1]
            gripper_euler[2] = tmp_euler[0]
...
def arm_move(rbt_s, onscreen, task):
    ...
    if operation_count == 0:
        ...
        current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=pre_tcp_pos + np.dot(adjustment_rotmat1, rel_tcp_pos) + agv_pos,
                                                                           tgt_rot=np.dot(modify_rot, rm.rotmat_from_euler(current_tcp_rot_euler[0], current_tcp_rot_euler[1], current_tcp_rot_euler[2])),
