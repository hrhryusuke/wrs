...
def display(attached_list, rbt_s, onscreen, pre_pos, task):
    global k, pre_manipulability, current_manipulability, manipulability_displacement, data_array, pre_rgt_hand_info, current_rgt_hand_info, \
        current_jnt_values_rgt, displacement, pre_jnt_displacement, fail_IK_flag, operation_count,pre_jnt_values_rgt, init_error_rgt, test_arg1
    ...
    if operation_count == 0:
        init_error = data_array[rgt_hand_num:(rgt_hand_num+3)] - [0.4, 0, 0.95]
        ...
        test_euler = rm.quaternion_to_euler(data_array[(rgt_hand_num + 6):(rgt_hand_num + 10)])
        print(test_euler)
        # 手先姿勢１
