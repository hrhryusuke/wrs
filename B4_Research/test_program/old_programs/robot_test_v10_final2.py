...
def data_adjustment(data_list):
    ...
    for i in range(0, tmp_data_array.size):
        if (i % 16) == 2: # センサ座標調整
            ...
            data_array[i-3: i+1] = rm.quaternion_from_matrix(cvt_rot_WtoM(adjustmented_rot1))
        else:
...
pre_sensor_quarternion = np.zeros((3, 3))
pre_rot = np.zeros((3, 3))
# アーム動作用関数
def arm_move(rbt_s, onscreen, task):
    global data_array, current_jnt_values_rgt, pre_jnt_values_rgt, fail_IK_flag, operation_count, modify_rot, modify_rot_for_pos, pre_tcp_pos, pre_sensor_pos_value, pre_sensor_quarternion, pre_rot
    ...
    if operation_count == 0:
        ...
        pre_sensor_qua = data_array[(rgt_hand_num+6):(rgt_hand_num+10)]
        if np.linalg.norm(rel_tcp_euler, ord=2) > 0.002:
            ...
            #                                                                    tgt_rot=np.dot(np.dot(modify_rot, rm.rotmat_from_euler(ai=rel_tcp_euler[0], aj=rel_tcp_euler[1], ak=rel_tcp_euler[2])), pre_rot),
            #                                                                    seed_jnt_values=pre_jnt_values,
            #                                                                    tcp_jntid=7)
            # # pre_jnt_values = current_jnt_values
            # pre_tcp_pos += np.dot(modify_rot_for_pos, rel_tcp_pos)
            # pre_rot = np.dot(np.dot(modify_rot, rm.rotmat_from_euler(ai=rel_tcp_euler[0], aj=rel_tcp_euler[1], ak=rel_tcp_euler[2])), pre_rot)
            current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=pre_tcp_pos + np.dot(modify_rot_for_pos, rel_tcp_pos) + agv_pos,
                                                                               tgt_rot=np.dot(rm.rotmat_from_euler(ai=math.radians(rel_tcp_euler[1]), aj=math.radians(-rel_tcp_euler[0]), ak=math.radians(rel_tcp_euler[2])), pre_rot),
                                                                               seed_jnt_values=pre_jnt_values,
                                                                               tcp_jntid=7)
            # pre_jnt_values = current_jnt_values
            pre_tcp_pos += np.dot(modify_rot_for_pos, rel_tcp_pos)
            pre_rot = np.dot(rm.rotmat_from_euler(ai=rel_tcp_euler[1], aj=-rel_tcp_euler[0], ak=rel_tcp_euler[2]), pre_rot)
        else:
            current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=pre_tcp_pos + np.dot(modify_rot_for_pos, rel_tcp_pos) + agv_pos,
                                                                               tgt_rot=pre_rot,
                                                                               seed_jnt_values=pre_jnt_values,
        ...
        if (current_jnt_values is not None) and (collided_result == False):
