...
def cvt_rot_W_to_tcp(World_rot):
    gripper_euler = np.zeros(3)

    adjustmented_rot2 = np.dot(adjustment_rotmat2, World_rot)
...
def arm_move(rbt_s, onscreen, task):
    global data_array, current_jnt_values_rgt, pre_jnt_values_rgt, fail_IK_flag, operation_count, modify_rot, modify_rot_for_pos, pre_tcp_pos, pre_sensor_pos_value, operator_coordinate, \
        init_error_rgt, pre_sensor_quarternion, pre_tcp_rot, init_tcp_rot, init_hand_rot
    ...
    if operation_count == 0:
        ...
        pre_sensor_qua = data_array[(rgt_hand_num + 6):(rgt_hand_num + 10)]
        if np.linalg.norm(rel_world_euler, ord=2) > 0.01:
            rel_tcp_rot = np.dot(np.linalg.pinv(operator_coordinate), rm.rotmat_from_euler(ai=rel_world_euler[0], aj=rel_world_euler[1], ak=rel_world_euler[2]))
            current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=np.dot(np.linalg.pinv(operator_coordinate), data_array[(rgt_hand_num):(rgt_hand_num + 3)]) - init_error + agv_pos,
                                                                               tgt_rot=np.dot(rel_tcp_rot, pre_tcp_rot),
                                                                               seed_jnt_values=pre_jnt_values,
                                                                               tcp_jntid=7)
            pre_jnt_values = current_jnt_values
            pre_tcp_rot = np.dot(cvt_rot_W_to_tcp(rel_tcp_rot), pre_tcp_rot)
        else:
            current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=np.dot(np.linalg.pinv(operator_coordinate), data_array[(rgt_hand_num):(rgt_hand_num + 3)]) - init_error + agv_pos,
                                                                               tgt_rot=pre_tcp_rot,
                                                                               seed_jnt_values=pre_jnt_values,
        ...
        if (current_jnt_values is not None) and (pre_jnt_values is not None) and (collided_result == False):
            if abs(np.linalg.norm(current_jnt_values-pre_jnt_values, ord=2)) <= threshold_jnt_values_displacement_norm:
