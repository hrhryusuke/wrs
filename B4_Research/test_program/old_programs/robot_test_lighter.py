...
import robot_sim.robots.xarm7_shuidi_mobile.xarm7_shuidi_mobile as xasm

# ロボットモデルを読み込み
rbt_s = xasm.XArm7YunjiMobile()
...
# 右手座標記録用配列
right_hand_info = np.zeros(3)

# データ抽出を行う
def update(s, attached_list, rbt_s, onscreen, zero_pos, task):
    ...
    if operation_count % 8 == 0:
        ...
        # 右手座標情報を記録
        tmp_right_hand_info = tmp_data_array[11*16:11*16+3]
        # print(tmp_right_hand_info)

        # 座標調整
        # right_hand_info = np.zeros(3)
        right_hand_info[0] = tmp_right_hand_info[0] * (-1)
        right_hand_info[1] = tmp_right_hand_info[1]
        right_hand_info[2] = tmp_right_hand_info[2] * (-1)
        ...
        if operation_count == 0:
            # 各部の座標情報を赤色の球として表示
            attached_list.append(gm.gen_sphere(pos=right_hand_info))
            attached_list[-1].attach_to(base)
            # print(attached_list)

            # 最初の時刻の右手の座標を基準値として設定
            zero_pos = right_hand_info
        else:
            # 球の位置情報を更新
            ...
            # 球の位置情報を更新
            # npvec3 = right_hand_info
            attached_list[-1]._objpdnp.setPos(right_hand_info[0], right_hand_info[1], right_hand_info[2])
            # 右手の座標を示す球は緑色に設定
            # attached_list[-1].set_rgba([0, 1, 0, 1])
            # armの位置と回転行列を取得
            tcp_pos, tcp_rotmat = rbt_s.manipulator_dict["arm"].get_gl_tcp()
            # 初期の右手の位置との差分を動きに反映させる
            new_tcp_pos = tcp_pos + arm_update_weight * (right_hand_info - zero_pos)
            # 逆運動学によって目標位置におけるジョイントの角度を計算
            jnt_values = rbt_s.ik("arm", new_tcp_pos, tcp_rotmat)
            # 順運動学で先ほどのIKの計算結果を適用（IKが解けないときはモデルを更新しない）
...
taskMgr.doMethodLater(update_frequency, update, "update",
                      extraArgs=[s, attached_list, rbt_s, onscreen, zero_pos],
                      appendTask=True)
base.run()
