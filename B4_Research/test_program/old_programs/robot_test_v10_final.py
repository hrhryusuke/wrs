...
threshold_jnt_values_displacement_norm = 0.13
...
def arm_move(rbt_s, onscreen, task):
    global data_array, current_jnt_values_rgt, pre_jnt_values_rgt, fail_IK_flag, operation_count, modify_rot, modify_rot_for_pos, pre_tcp_pos, pre_sensor_pos_value
    ...
    if operation_count == 0:
        ...
        current_jnt_values = rbt_s.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=pre_tcp_pos + np.dot(modify_rot_for_pos, rel_tcp_pos) + agv_pos,
                                                                           tgt_rot=np.dot(modify_rot, rm.rotmat_from_quaternion(data_array[(rgt_hand_num+6):(rgt_hand_num+10)])[:3, :3]),
