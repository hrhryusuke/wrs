"""
---dual_operate_xarm_sim_v1.0概要---
Axis NEURONから送信されてきたPERCEPTION NEURONの右手の位置姿勢情報を元にシミュレータ上の２台のxarmに動きを同期させる
...
rbt_s_rgt = xarm.XArmShuidi(enable_cc=True) # ロボットモデルの読込
rbt_s_lft = xarm.XArmShuidi(enable_cc=True)
...
if __name__ == '__main__':
    ...
    # right
    sim_init_pos_rgt, sim_init_rot_rgt = rbt_s_rgt.get_gl_tcp()
    sim_init_pos_rgt[2] = sim_init_pos_rgt[2] - 0.2 # NOTICE:高さを変えないとikの解が導出不可
    ...
    # left
    sim_init_pos_lft, sim_init_rot_lft = rbt_s_lft.get_gl_tcp()
    sim_init_pos_lft[2] = sim_init_pos_lft[2] - 0.2  # NOTICE:高さを変えないとikの解が導出不可
    ...
    # rbt_s_rgt.gen_meshmodel().attach_to(base)
    # rbt_s_lft.gen_meshmodel().attach_to(base)
    # base.run()
