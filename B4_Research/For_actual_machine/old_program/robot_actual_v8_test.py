...
def arm_move(rbt_s_rgt, rbt_s_lft, onscreen_rgt, onscreen_lft, task):
    ...
    if operation_count == 0:
        ...
        pre_sensor_pos_value_rgt = data_array[rgt_hand_num:(rgt_hand_num+3)]
        current_jnt_values_rgt = rbt_s_rgt.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=pre_tcp_pos_rgt + np.dot(modify_rot_for_pos_rgt, rel_tcp_pos_rgt),
        ...
        pre_tcp_pos_rgt += np.dot(modify_rot_for_pos_rgt, rel_tcp_pos_rgt)
        # left_arm
        tcp_pos_lft, tcp_rot_lft = rbt_s_lft.manipulator_dict['arm'].jlc.get_gl_tcp()
        ...
        pre_sensor_pos_value_lft = data_array[lft_hand_num:(lft_hand_num + 3)]
        current_jnt_values_lft = rbt_s_lft.manipulator_dict['arm'].jlc._ikt.num_ik(tgt_pos=pre_tcp_pos_lft + np.dot(modify_rot_for_pos_lft, rel_tcp_pos_lft),
        ...
        pre_tcp_pos_lft += np.dot(modify_rot_for_pos_lft, rel_tcp_pos_lft)
        ...
        if (current_jnt_values_rgt is not None) and (pre_jnt_values_rgt is not None) and (collided_result_rgt == False):
            if abs(np.linalg.norm(current_jnt_values_rgt - pre_jnt_values_rgt, ord=2)) <= threshold_jnt_values_displacement_norm:
        ...
        if (current_jnt_values_lft is not None) and (pre_jnt_values_lft is not None) and (collided_result_lft == False):
            if abs(np.linalg.norm(current_jnt_values_lft - pre_jnt_values_lft, ord=2)) <= threshold_jnt_values_displacement_norm:
